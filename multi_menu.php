...
/**
 * Codeigniter 4 Multilevel menu Class
 * Provide easy way to render multi level menu
 * 
 * @package			CodeIgniter
 * @subpackage		Libraries
 * @category		Libraries
 * @author			Muhamad Rizki Septiawan  inspired by Eding Muhamad Saprudin (https://github.com/edomaru/codeigniter_multilevel_menu)
 * @link    		https://github.com/emrizkis/codeigniter4_multilevel_menu
 */
class Multi_menu
{
	...
	/**
	 * Anchor tag of the header menu
	 * default is '<a href="%s">%s</a>'
	 * 
	 * @var string
	 */
	private $menu_header_anchor            = '<a href="%s">%s</a>';

	/**
	 * Anchor tag of the item header menu
	 * default is '<a href="%s">%s</a>'
	 * 
	 * @var string
	 */
	private $item_header_anchor            = '<a href="%s">%s</a>';
	 ...
	 * @var string
	 */
	// private $parentl1_tag_open        = '<li class="nav-item dropdown dropdown-hover ml-1">';
	private $parent_header_tag_open   = '<li>';
	 ...
	 * @var string
	 */
	// private $parentl1_anchor          = '<a role:"button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle " href="%s">%s</a>';
	...
	public function __construct($config = array())
	{
		...
		require_once APPPATH . "\libraries\multilevel_menu\config.php";
	 ...
	 * @return void         
	 */
	private function render_item($items, &$html = '', $level = 0)
	{
		$tempLevel = $level;
		if (empty($html)) {
			$nav_tag_opened = true;
			...
			if (isset($item[$this->menu_label], $item[$this->menu_key])) {
				...
				if ($has_children) {
					if (is_null($item[$this->menu_parent]) && $this->parentl1_tag_open != '') {
						$tag_open    = $level !== 0 ? $this->parent_header_tag_open : $this->parentl1_tag_open;
						$item_anchor = $this->parentl1_anchor != '' ?  $this->menu_header_anchor : $this->parent_anchor;
					} else {
						$tag_open     =   $level !== 0 ? $this->parent_header_tag_open : $this->parent_tag_open;
						// $tag_open     =  $this->parent_tag_open;
						// $item_anchor = $this->parentl1_anchor != '' ? $this->parentl1_anchor . "cek" : $this->parent_anchor . "cek";
						$item_anchor = $level === 0 ? $this->menu_header_anchor : $this->parent_anchor;
						// $item_anchor = $level === 0 ? $this->menu_header_anchor : $this->subheader_anchor;
					}

					$href  = '#';
				} else {
					$tag_open    =  $level === 0 ? $this->parent_header_tag_open : $this->item_tag_open;

					$href        = (!empty(strpos((string)$slug, ".php"))) ? getenv("PARENT_APP") . $slug : base_url($slug);
					// $item_anchor = $this->item_anchor;
					$item_anchor = $level === 0 ? $this->item_header_anchor : $this->item_anchor;
				}
				...
				if ($has_children) {
					$level++;
					$this->render_item($item['children'], $html, $level);
	...
	{
		$segment = base_url(); //$this->ci->uri->segment($this->uri_segment);
